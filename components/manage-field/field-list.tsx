...
export const ListField = () => {
    ...
    useEffect(() => {
        const fetchDocuments = async () => {
            setLoading(true);
            try {
                const response = await axios.get("/api/fieldDocument", {
                    params: {
                        page: currentPage,
                        keyword: search,
                ...
                setListLinhVuc(response.data.linhVuc);
    ...
    return (
        <div>
            <div className="flex flex-row gap-4 justify-between">
                <Button className="mt-5" variant={"primary"} onClick={() => onOpen("createFieldDocument")}>
                    <span className="pl-2">Thêm mới</span>
                </Button>
                <div className="relative flex items-center w-1/5 py-4 bg-white dark:bg-gray-800">
                    <Input
                        placeholder="Nhập mã, tên lĩnh vực hoăc mô tả"
            ...
            <Table>
                ...
                <TableBody>
                    {loading ? (
                        ...
                        listLinhVuc.map((field: any, index: number) => (
                            <TableRow key={field.ma}>
                                ...
                                <TableCell className="text-center">
                                    <div>
                                        <Button className="mr-2" variant={"outline"} onClick={() => onOpen("editFieldDocument", field)}>Sửa</Button>
